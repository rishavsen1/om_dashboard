...
def generate_hvac_load_profile(
    hvac_consumption: float,
    hvac_peak_time: int,
    hvac_load_shape: int
) -> List[float]:
    """
...
def calculate_homeowner_savings(params: Dict) -> Dict:
    ...
    hvac_usage = generate_hvac_load_profile(
        hvac_consumption, hvac_peak_time, hvac_load_shape
    )
    
    # Get hourly rates
    rates = calculate_hourly_rates(
        pricing_model, peak_rate, off_peak_rate,
        peak_start, peak_end, hourly_prices
    )
    ...
    peak_cost_no_battery = sum(
        hvac_usage[h] * rates[h] for h in range(24)
    ...
    battery_result = optimize_battery_dispatch(
        hvac_usage, rates, battery_capacity, min_soc, max_soc,
        discharge_duration, battery_power, battery_efficiency
    )
    ...
    hvac_from_grid = [
        usage - discharge
        for usage, discharge in zip(hvac_usage, discharge_plan)
    ]
    
    peak_cost_with_battery = sum(
        hvac_from_grid[h] * rates[h] for h in range(24)
        if peak_start <= h < peak_end
    )
    off_peak_cost_with_battery = sum(
        hvac_from_grid[h] * rates[h] for h in range(24)
        if not (peak_start <= h < peak_end)
    )
...
def calculate_ci_value(params: Dict) -> Dict:
    ...
    for load_mw in project_loads:
        ...
        npv_with_vpp = sum(
            total_ebitda / ((1 + discount_rate) ** year)
            for year in range(1, op_horizon + 1)
        )
        
        # Calculate NPV without VPP (delayed start)
        npv_without_vpp = sum(
            total_ebitda / ((1 + discount_rate) ** (year + time_savings))
            for year in range(1, op_horizon + 1)
        )
